...
function irecommendthis_enqueue_editor_assets() {
	...
	if ( 0 === $disable_css ) {
		...
		$css_url  = plugins_url( 'assets/css/' . $css_file, dirname( __FILE__ ) );
		...
		wp_register_style(
			...
			$css_url,
			array(),
			defined( 'THEMEIST_IRT_VERSION' ) ? THEMEIST_IRT_VERSION : '4.0.0'
		);
...
// Add the action hook for editor assets
add_action( 'enqueue_block_editor_assets', 'irecommendthis_enqueue_editor_assets' );
